...
public class SheetBar extends HorizontalScrollView implements OnClickListener
{
    ...
    private void init()
    {
        ...
        sheetbarFrame.setMinimumWidth(minimumWidth == -1 ? getResources().getDisplayMetrics().widthPixels
            : minimumWidth);
        sheetbarHeight = 100;//drawable.getIntrinsicHeight();
        
        drawable = sheetbarResManager.getDrawable(SheetbarResConstant.RESID_SHEETBAR_SHADOW_LEFT);
        LayoutParams parmas = new LayoutParams(LayoutParams.WRAP_CONTENT, sheetbarHeight);
        // 左边shadow
...
//        left.setBackgroundDrawable(drawable);
        ...
        LayoutParams parmasButton = new LayoutParams(LayoutParams.WRAP_CONTENT, sheetbarHeight);
        parmasButton.setMargins(20,0,20,0);
        int count = vec.size();
